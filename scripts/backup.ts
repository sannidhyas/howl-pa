...
function backup(outArg: string | undefined): void {
...
function normalizeArchiveEntry(entry: string): CredentialFile | null {
  let normalized = entry.trim()
  while (normalized.startsWith('./')) normalized = normalized.slice(2)
  if (CREDENTIAL_FILES.includes(normalized as CredentialFile)) {
    return normalized as CredentialFile
  }
  return null
}

function listRestorableFiles(archivePath: string): CredentialFile[] {
  ...
  const included = new Set<CredentialFile>()
