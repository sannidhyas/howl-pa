...
import { VAULT_SUBDIRS, appendToDailySection, nowStamp, serializeFrontmatter, todayIso, vaultPath, vaultRelFrom, } from './vault.js';
...
async function bot_commit(vaultRelPath, op) {
    try {
...
export async function writeLiterature(input) {
    const slug = slugify(input.slug ?? input.title ?? input.text.slice(0, 60));
...
export async function writeThesisFragment(input) {
    const stamp = nowStamp();
...
export async function writeIdea(input) {
    const date = todayIso();
    ...
    const seedContent = serializeFrontmatter({
        created: date,
        idea: input.slug,
        status: 'not-opened',
