 ...
 */
export function formatRecallHtml(hits, query) {
    if (hits.length === 0) {
    ...
    const body = hits
        ...
        return (`${kindBadge(h.kind)} <b>${i + 1}.</b> <code>${refEsc}</code>` +
            ` · <i>${scoreStr}</i>` +
            (prov ? ` <i>(${prov})</i>` : '') +
            `\n${escapeHtml(preview)}`);
    })
...
export function formatReindexResultHtml(res) {
    return (`<b>Vault reindex</b> · scanned <b>${res.scanned}</b>` +
        ` · indexed <b>${res.reindexed}</b>` +
        ` · chunks <b>${res.chunksStored}</b>` +
        ` · skipped ${res.skipped}` +
        ` · errors ${res.errored}`);
}
//# sourceMappingURL=format-telegram.js.map