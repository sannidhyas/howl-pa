...
import { listCalendarEventsBetween, upsertCalendarEvent } from './db.js';
import { logger } from './logger.js';
export async function isCalendarReady() {
    return googleAuthConfigured() && googleTokenSaved();
}
export async function pollCalendar(lookAheadHours = 36) {
...
function extractMeetLink(ev) {
    ...
    for (const p of ep)
        if (p.entryPointType === 'video' && p.uri)
            return p.uri;
    return undefined;
}
export function calendarBetween(fromMs, toMs) {
