...
async function promptValue(question, hidden = false) {
...
async function ensureClientCreds() {
    ...
    if (!process.env.GOOGLE_CLIENT_ID) {
        ...
        if (!id) {
            console.error('aborted — client id required');
            process.exit(1);
        }
    ...
    if (!process.env.GOOGLE_CLIENT_SECRET) {
        ...
        if (!secret) {
            console.error('aborted — client secret required');
            process.exit(1);
        }
...
const { authUrl, exchangeCode, googleAuthConfigured, googleTokenSaved } = await import('../src/google-auth.js');
function renderOAuthPage(args) {
