...
function roleFor(model) {
    ...
    if (lower.includes('coder'))
        return 'code correctness and implementation risk reviewer';
    if (lower.includes('ministral') || lower.includes('mistral'))
        return 'product, planning, and failure-mode reviewer';
    if (lower.includes('30b') || lower.includes('qwen3'))
        return 'architecture and systems reviewer';
...
async function memberReview(model, task, body) {
...
async function synthesize(task, members) {
    const ok = members.filter(m => m.text.trim().length > 0 && !m.error);
    if (ok.length === 0)
        return 'All Ollama council members failed.';
...
async function runCouncil(task, body) {
...
function formatReport(task, synthesis, members) {
...
function replaceCouncilBlock(original, report) {
    const block = [
...
async function review() {
    ...
    if (!body.trim())
        throw new Error('review mode expects stdin content');
...
async function bounce(planFile, passesRaw) {
    if (!planFile)
        throw new Error('bounce mode expects PLAN.md path');
...
//# sourceMappingURL=ollama-council.js.map