...
function gitSummary(path) {
    ...
    if (res.status !== 0)
        return `git status failed: ${(res.stderr || res.stdout).trim()}`;
...
function checkDb() {
    ...
    const running = db.prepare(`SELECT name, last_run FROM scheduled_tasks WHERE status='running' ORDER BY name`).all();
    const stale = running.filter(r => !r.last_run || Date.now() - r.last_run > AGENT_TIMEOUT_MS);
    add('scheduler running', stale.length > 0 ? 'fail' : running.length > 0 ? 'warn' : 'ok', stale.length > 0 ? `stale: ${stale.map(r => r.name).join(', ')}` : `${running.length} running`);
    const tasks = db.prepare(`SELECT status, COUNT(*) AS n FROM scheduled_tasks GROUP BY status ORDER BY status`).all();
    add('scheduled tasks', 'ok', tasks.map(t => `${t.status}:${t.n}`).join(', ') || 'none');
    const chunks = db.prepare(`SELECT COUNT(*) AS n FROM memory_chunks`).get().n;
