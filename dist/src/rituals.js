...
function normaliseYesNo(raw) {
    ...
    if (['y', 'yes', 'done', '1', 'true'].includes(v))
        return 'true';
    if (['n', 'no', 'skip', 'missed', '0', 'false'].includes(v))
        return 'false';
    return null;
}
export async function openMorningSurvey(chatId, send) {
    startSurvey({ kind: 'morning', chatId, step: 0, started: Date.now(), data: {} });
    await send(MORNING_QUESTIONS[0].prompt);
}
export async function openEveningSurvey(chatId, send) {
    startSurvey({ kind: 'evening', chatId, step: 0, started: Date.now(), data: {} });
    await send(EVENING_QUESTIONS[0].prompt);
}
export async function openWeeklyReviewSurvey(chatId, send) {
...
async function applyEveningToDaily(isoDate, data) {
    ...
    if (data.energy)
        fm.energy = data.energy;
    if (data.soreness)
        fm.soreness = data.soreness;
    if (data.swim)
        fm.swim = data.swim === 'true';
    if (data.sport && data.sport.toLowerCase() !== 'none')
        fm.sport = data.sport;
    if (data.kit_done)
        fm.kit_done = data.kit_done === 'true';
    if (data.meditation_done)
        fm.meditation_done = data.meditation_done === 'true';
    const next = serializeFrontmatter(fm) + '\n' + (body.startsWith('\n') ? body.slice(1) : body);
...
export async function handleSurveyReply(chatId, text, send) {
    ...
    if (survey.kind === 'morning') {
        ...
        if (q.key === 'focus' && raw.toLowerCase() !== 'skip') {
            ...
            for (const line of lines)
                await appendToDailySection(today, 'Focus (pick 1–3)', `- ${line}`);
        }
    ...
    if (survey.kind === 'weekly-review') {
        ...
        const queue = (() => {
            try {
                return JSON.parse(survey.data.queue ?? '[]');
            }
            catch {
                return [];
            }
...
async function createNeedleTasks(isoDate, raw) {
    const lines = raw.split(/\n+/).map(l => l.trim()).filter(Boolean).slice(0, 3);
...
function dueAt(isoDate, hour, minute) {
