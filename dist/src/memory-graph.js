...
export function listVaultNotes(subdir) {
    ...
    function walk(dir) {
        ...
        for (const name of entries) {
            if (name.startsWith('.'))
                continue;
            if (name === 'node_modules' || name === 'Attachments' || name === '411_Attachments')
                continue;
...
export function findBacklinksFor(noteName) {
...
export function readNoteBody(absPath) {
    const text = readFileSync(absPath, 'utf8');
    const fm = FRONTMATTER_RE.exec(text);
    const frontmatter = {};
    if (fm && fm[1]) {
