...
export async function routeDelegation(d, chatId) {
    ...
    const outcome = await dispatchSubagent({ prompt: header + d.prompt, chatId, hints: [] }, { mode: 'single' });
    const duration = Date.now() - start;
    // hive_mind — log every cross-agent interaction so other agents can see recent activity.
    try {
        getDb()
            .prepare(`INSERT INTO hive_mind (agent_id, chat_id, action, summary, artifacts)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT DO NOTHING`)
            .run(d.agentId, chatId, 'delegation', d.prompt.slice(0, 240), outcome.final.slice(0, 600));
    }
    catch (err) {
...
export function ensureHiveMindSchema() {
    getDb().exec(`
