...
function findPipelineIdeaDir(slug) {
...
async function rewriteIndexStatus(indexAbs, projectRel, projectNumber) {
    ...
    if (!tagList.includes('opened'))
        tagList.push('opened');
    const idxParked = tagList.indexOf('parked');
    if (idxParked >= 0)
        tagList.splice(idxParked, 1);
    fm.tags = tagList;
    return serializeFrontmatter(fm) + '\n\n' + body.replace(/^\s*/, '');
}
export async function openIdea(slug, overrideName) {
    const pipelineDir = findPipelineIdeaDir(slug);
    if (!pipelineDir)
        throw new Error(`pipeline idea not found: ${slug}`);
    const indexPath = join(pipelineDir, 'index.md');
    if (!existsSync(indexPath))
        throw new Error(`missing index.md in ${pipelineDir}`);
    ...
    if (existsSync(projectAbs))
        throw new Error(`target exists: ${projectAbs}`);
    ...
    await appendToDailySection(todayIso(), 'Venture artifact (one tangible thing)', `- opened [[${vaultRelFrom(newIndexAbs)}|${rawTitle}]] (${projectName})`);
    try {
...
export async function discardIdea(slug) {
    const pipelineDir = findPipelineIdeaDir(slug);
    if (!pipelineDir)
        throw new Error(`pipeline idea not found: ${slug}`);
    ...
    if (existsSync(destAbs))
        throw new Error(`archive target exists: ${destAbs}`);
...
export function listParkedIdeas(limit = 40) {
    const root = vaultPath(VAULT_SUBDIRS.pipeline, 'ideas');
    if (!existsSync(root))
        return [];
    const out = [];
...
//# sourceMappingURL=idea-open.js.map