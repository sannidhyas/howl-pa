...
export async function embedderHealthy(force = false) {
    ...
    try {
        ...
        if (!res.ok)
            _lastErrMsg = `health check HTTP ${res.status}`;
...
export async function embed(text) {
    ...
    if (!(await embedderHealthy())) {
        ...
        if (!_down) {
            logger.warn({ url: OLLAMA_URL, lastError }, 'ollama embedder down — entering cooldown');
        }
        else if (lastError !== previousError) {
            logger.warn({ url: OLLAMA_URL, lastError }, 'ollama embedder still down — cooldown reset');
        }
        _down = true;
