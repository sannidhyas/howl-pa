...
function builtInsForProfile(profile) {
...
function registerBuiltIns(profile) {
    ...
    for (const b of builtInsForProfile(profile)) {
        ...
        if (prev && prev.schedule === b.schedule)
            continue;
        upsertScheduledTask({
            name: b.name,
            mission: b.mission,
...
export function initScheduler(opts) {
    ...
    state.timer.unref();
    logger.info({ profile, built_ins: builtInsForProfile(profile).map(b => b.name) }, 'scheduler started');
}
