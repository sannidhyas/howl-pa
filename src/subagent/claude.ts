...
export class ClaudeBackend implements SubagentBackend {
  ...
  async run(input: SubagentInput): Promise<SubagentResult> {
    ...
    try {
      for await (const msg of query({ prompt: input.prompt, options }) as AsyncIterable<SDKMessage>) {
        if (msg.type === 'system' && msg.subtype === 'init') result.sessionId = msg.session_id
        if (msg.type === 'result') {
