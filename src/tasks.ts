...
export function isTasksReady(): boolean {
  return googleAuthConfigured() && googleTokenSaved()
}

export function queueTask(input: GoogleTaskInput): TaskItemRow {
...
export async function upsertTask(input: GoogleTaskInput): Promise<TaskItemRow> {
  if (!isTasksReady()) {
  ...
  const task = res.data
  if (!task.id) throw new Error('Google Tasks insert returned no id')
...
export async function pushPendingTasks(limit = 25): Promise<TaskSyncResult> {
  if (!isTasksReady()) return { ok: false, reason: 'not configured', fetched: 0, stored: 0, pushed: 0 }
  const pending = pendingTaskItems(limit)
  if (pending.length === 0) return { ok: true, fetched: 0, stored: 0, pushed: 0 }
  let pushed = 0
...
export async function pollTasks(): Promise<TaskSyncResult> {
  if (!isTasksReady()) return { ok: false, reason: 'not configured', fetched: 0, stored: 0 }
  const auth = await getAuthedClient()
  const service = google.tasks({ version: 'v1', auth })
...
export function taskRows(limit = 20): TaskItemRow[] {
...
function mapTask(task: { id?: string | null; title?: string | null; notes?: string | null; due?: string | null; status?: string | null; updated?: string | null }, listId: string): TaskItemRow {
...
function toUpsert(row: TaskItemRow): Parameters<typeof upsertTaskItem>[0] {
  return {
    id: row.id,
    listId: row.list_id,
    title: row.title,
