...
export async function replaceChunksFor(
  sourceKind: MemorySourceKind,
  sourceRef: string,
  chunks: string[],
  mtime?: number
...
export async function queryCosineTopK(
  queryText: string,
  k = 5,
  filter?: { kind?: MemorySourceKind }
): Promise<VectorHit[]> {
  const qvec = await embed(queryText)
  if (!qvec) return []
  const rows = allMemoryChunks(filter?.kind)
  const scored: VectorHit[] = rows.map((row: MemoryChunkRow) => ({
    id: row.id,
