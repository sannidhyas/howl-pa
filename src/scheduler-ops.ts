...
const { parseExpression } = CronParserModule as unknown as { parseExpression: (expr: string) => { next(): { getTime(): number } } }
import { MISSIONS } from './missions/index.js'
import { nextRunFor } from './scheduler.js'
import { upsertScheduledTask, updateScheduledFields, listScheduledTasks, type ScheduledTaskStatus } from './db.js'

const NAME_RE = /^[a-z0-9_-]{1,64}$/

type CreateRoutineResult = { ok: true; next_run: number } | { ok: false; error: string }
type EditRoutineResult =
  | { ok: true; updated: string; next_run?: number }
...
export async function createRoutine(input: {
  ...
  if (!NAME_RE.test(name)) return { ok: false, error: 'invalid name format' }
  if (listScheduledTasks().some(t => t.name === name)) return { ok: false, error: 'name exists' }
  ...
  if (!schedule) return { ok: false, error: 'schedule required' }
  const cErr = cronError(schedule)
  if (cErr) return { ok: false, error: `invalid cron: ${cErr}` }
...
export async function editRoutine(name: string, field: string, rawValue: string): Promise<EditRoutineResult> {
  const routineName = typeof name === 'string' ? name.trim() : ''
  if (!NAME_RE.test(routineName)) return { ok: false, error: 'invalid name format' }
  ...
  if (key === 'priority') {
    if (!/^-?\d+$/.test(value)) return { ok: false, error: 'priority must be an integer from 0 to 100' }
  ...
  if (key === 'args') {
    ...
    if (!validObject(parsed)) return { ok: false, error: 'args must be object' }
  ...
  const status = value as ScheduledTaskStatus
