...
function splitMessage(text: string, limit = MAX_TEXT): string[] {
...
function isAuthorised(chatId: string | number): boolean {
  return String(chatId) === String(ALLOWED_CHAT_ID)
}

async function handleSpecialistMessage(ctx: Context, agentId: string, text: string): Promise<void> {
...
export function makeSpecialistBot(agentId: string, token: string): AgentBotHandle {
...
export async function startAllSpecialistBots(): Promise<AgentBotHandle[]> {
  ensureHiveMindSchema()
  const tokens = discoverAgentBotTokens()
  const handles: AgentBotHandle[] = []
  for (const { agentId, token } of tokens) {
    if (!agentExists(agentId)) {
      logger.warn(
        { agentId },
        `TELEGRAM_BOT_TOKEN_${agentId.toUpperCase()} set but no agent scaffold. Run \`npm run howl agent:create ${agentId}\`.`
      )
      continue
    }
