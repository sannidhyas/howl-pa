...
async function runClaudeSummary(prompt: string): Promise<string> {
  ...
  try {
    for await (const message of query({ prompt, options }) as AsyncIterable<SDKMessage>) {
      if (message.type === 'result' && message.subtype === 'success') out = message.result
    }
  } finally {
    clearTimeout(timer)
  }
...
export async function runIdeaFlow(input: IdeaInput): Promise<{ indexRel: string; seedRel: string; title: string }> {
