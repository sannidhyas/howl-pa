...
import {
  ALLOWED_CHAT_ID,
  DASHBOARD_HOST,
  DASHBOARD_USERNAME as DASHBOARD_USERNAME_DEFAULT,
  DASHBOARD_PASSWORD_HASH as DASHBOARD_PASSWORD_HASH_BOOT,
...
const MEMORY_SCOPES = ['global', 'email_hint', 'task_hint', 'agent_hint', 'capture_hint', 'journal_hint'] as const
...
function verifyPassword(username: string, password: string): boolean {
  ...
  if (username !== liveUsername()) return false
...
function buildApp(): Hono {
  ...
  app.get('/api/scheduler', c => {
    ...
    const data = rows(
      `SELECT id, name, mission, schedule, next_run, last_run, last_result, priority, status, muted
  ...
  app.get('/api/memory', c => {
    ...
    const memoryRows: SystemMemoryRow[] = listMemories(scopeRaw)
